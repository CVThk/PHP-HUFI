...
<?php
    ...
    $idsp = $_GET["masp"];
    $sp = $db->executeReader("SELECT * FROM `sanpham`, `loai` WHERE `sanpham`.`MaLoai` = `loai`.`MaLoai` and  MaSP = ?", array($idsp))[0];
?>
        ...
        <div class="row">
            ...
            <div class="col col-md-8"  style="background-color:lavender;">
                ...
                <div style="padding: 0 10px;">
                    <div style="text-transform: uppercase; font-size: 20px; font-weight: bold; color:darkblue;">thông tin chi tiết sản phẩm</div>
                    <div class="row" style="margin-top: 20px; background-color: #fff; border: 1px solid #333; padding:10px;">
                        <div class="col-4">
                            <img style="height: 200px;" src="./Images_KTL2/<?php echo $sp->Anh ?>"/>
                            <div style="display: flex; justify-content: space-between; margin-top: 10px; text-transform: uppercase;">
                                <a href="ThemVaoGioHang.php?masp=<?php echo $sp->MaSP ?>&amount=1" class="btn btn-primary" style="color: #fff;">Mua ngay</a>
                                <a href="ShowProduct.php" class="btn btn-primary" style="color: #fff;">Mua tiếp</a>
                            </div>
                        </div>
                        <div class="col-8">
                            <div style="display: flex; margin-top: 20px;">
                                <div style="width: 50%;">Tên sản phẩm:</div>
                                <span style="font-weight: bold; color:darkblue;"><?php echo $sp->TenSP ?></span>
                            </div>
                            <div style="display: flex; margin-top: 20px;">
                                <div style="width: 50%;">Hãng:</div>
                                <span style="color:darkblue;"><?php echo $sp->TenLoai ?></span>
                            </div>
                            <div style="display: flex; margin-top: 20px;">
                                <div style="width: 50%;">Mô tả kỹ thuật:</div>
                                <span><?php echo $sp->MoTa ?></span>
                            </div>
                            <div style="display: flex; margin-top: 20px;">
                                <div style="width: 50%;">Giá bán:</div>
                                <span style="font-weight: bold; color:red;"><?php echo number_format($sp->GiaBan, 0, ',', '.').' VNĐ' ?></span>
                            </div>
                            <div style="display: flex; margin-top: 20px;">
                                <div style="width: 50%;">Số lượng còn:</div>
                                <span style="font-weight: bold; color:darkblue;"><?php echo $sp->SLTon ?></span>
                            </div>
                        </div>
                    </div>
                </div>
